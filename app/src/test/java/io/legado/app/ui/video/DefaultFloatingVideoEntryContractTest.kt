...
class DefaultFloatingVideoEntryContractTest {
    ...
    fun `floating service enters foreground before an early return`() {
        ...
        assertTrue(baseOnStart.contains("promoteForegroundNotification()"))
        val foregroundPromote = baseService.substringAfter("protected fun promoteForegroundNotification()")
            .substringBefore("private fun tryStartForegroundNotification()")
        assertTrue(foregroundPromote.contains("tryStartForegroundNotification()"))
