...
| `https://199.33.126.51/` | **skip migrate** | 旧同人圈 IP 壳 L1 timeout；**25 本** remap→`https://m.tongrenquan.org`（同 `/tongren/{id}.html`）；禁 http/https IP 源；slash `m.tongrenquan.org/` oneshot 弄坏后禁用；非 slash **校验成功 2716ms**。 |
...
| `https://m.shouda88.com/` | **skip**（2026-08-10） | `deadish:this domain`；已禁用 |
| `https://m.paipaiwx.com` | **fixed**（2026-08-10） | 无搜索；打开路径 OK；发现校验成功 662ms；清 Error 备注 |
